...
namespace MemcardRex
{
	public partial class compareWindow : Form
	{
		...
		public void initializeDialog(mainWindow hostWindow, string appName, byte[] save1Data, string save1Title, byte[] save2Data, string save2Title)
		{
			...
			//Compare saves
			for (var i = 0; i < save1Data.Length; i++)
			{
				//Check if the bytes are different
				if (save1Data[i] != save2Data[i])
				{
					compareListView.Items.Add("0x" + i.ToString("X4") + " (" + i + ")");
					compareListView.Items[compareListView.Items.Count - 1].SubItems.Add("0x" + save1Data[i].ToString("X2") + " (" + save1Data[i] + ")");
					compareListView.Items[compareListView.Items.Count - 1].SubItems.Add("0x" + save2Data[i].ToString("X2") + " (" + save2Data[i] + ")");
				}
			}
