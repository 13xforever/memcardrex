...
namespace DexDriveCommunication
{
	internal class DexDrive
	{
		...
		public string StartDexDrive(string ComPortName)
		{
			...
			for (var i = 0; i < 10; i++) SendDataToPort((byte)DexCommands.MAGIC_HANDSHAKE, null, 0);
			Thread.Sleep(50);
		...
		public void StopDexDrive() { if (OpenedPort.IsOpen) OpenedPort.Close(); }
		...
		public byte[] ReadMemoryCardFrame(ushort FrameNumber)
		{
			...
			var XorData = (byte)(FrameLsb ^ FrameMsb); //XOR variable for consistency checking

			//Read a frame from the Memory Card
			SendDataToPort((byte)DexCommands.READ, new[] {FrameLsb, FrameMsb}, 0);
		...
		public bool WriteMemoryCardFrame(ushort FrameNumber, byte[] FrameData)
		{
			...
			var FrameLsb = (byte)(FrameNumber & 0xFF); //Least significant byte
			var FrameMsb = (byte)(FrameNumber >> 8); //Most significant byte
			var RevFrameLsb = ReverseByte(FrameLsb); //Reversed least significant byte
			var RevFrameMsb = ReverseByte(FrameMsb); //Reversed most significant byte
			var XorData = (byte)(FrameMsb ^ FrameLsb ^ RevFrameMsb ^ RevFrameLsb); //XOR variable for consistency checking
			...
			SendDataToPort((byte)DexCommands.WRITE, new[] {FrameMsb, FrameLsb, RevFrameMsb, RevFrameLsb}, 0); //Frame number
			OpenedPort.Write(FrameData, 0, FrameData.Length); //Save data
			OpenedPort.Write(new[] {XorData}, 0, 1); //XOR Checksum
